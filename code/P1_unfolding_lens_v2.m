...
if strcmp(dataType,'tif')
    [img,imgfolder] = load_tif_sequence(imgfolder);
    niiData = make_nii(img,[1 1 1],[0 0 0],16);
    save_nii(niiData,[saveDir 'original_image.nii.gz']);
elseif strcmp(dataType,'amira')
    [img,imgfolder] = load_amira(imgfolder,datafilename);
    niiData = make_nii(img,[1 1 1],[0 0 0],16);
    save_nii(niiData,[saveDir 'original_image.nii.gz']);
elseif strcmp(dataType,'nii')
    niiData = load_nii(fullfile(imgfolder,datafilename));
end
clear img
% read in the lables
fprintf('load label \n')
if strcmp(dataType,'tif')
    [Lbl,Lblfolder] = load_tif_sequence(Lblfolder);
    niiLbl = make_nii(Lbl,[1 1 1],[0 0 0],8);
    save_nii(niiLbl,[saveDir 'original_image.nii.gz']);
elseif strcmp(dataType,'amira')
    [Lbl,Lblfolder] = load_amira(Lblfolder,labelfilename);
    niiLbl = make_nii(Lbl,[1 1 1],[0 0 0],8);
    save_nii(niiLbl,[saveDir 'original_image.nii.gz']);
elseif strcmp(dataType,'nii')
