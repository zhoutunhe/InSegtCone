function varargout = multiscale_texture_gui(image,mappings,nr_labels,labeling)
%IMULTISCALE_TEXTURE_GUI   Multiscale version of texture gui
% MULTISCALE_TEXTURE_GUI(IMAGE,DICTOP,NR_LABELS,LABELING,)
% Input:
%   IMAGE, may be rgb or grayscale, 0-to-1 or uint8
%   MAPPINGS, struct containing multiscale mappings.
...
method_options = ...
    {'multiscale', 'multiscale_two', 'multiscale_multiply', 'two_weighted', 'two_weighted_multiply'};
...
% parsing_mapping_input; % T1 and T2 are assigned here
...
%%%%%%%%%% LABELINGS TO PROBABILITIES METHODS %%%%%%%%%%
    ...
    function [probabilities,dictprob] = multiscale(labelings)
        nr_scales = length(mappings);
        labelings(~any(labelings,2),:) = 1/nr_labels;
        label_image = reshape(labelings, [r,c,nr_labels]);
        prob_im = zeros(r,c,nr_labels);
        dictprob = cell(nr_scales,1);
        for i = 1:nr_scales
            label_image_sc = imresize(label_image, mappings(i).scale, 'nearest');
            [r_scale,c_scale,~] = size(label_image_sc);
            label_vec_sc = reshape(label_image_sc, [r_scale*c_scale, nr_labels]);
            
            dictprob{i} = mappings(i).T1*label_vec_sc;
            probabilities_sc = mappings(i).T2*dictprob{i}; % computing probabilities
            prob_im_sc = reshape(probabilities_sc, [r_scale, c_scale, nr_labels]);
            prob_im = prob_im + imresize(prob_im_sc, [r,c]);
        end
        prob_im = prob_im/nr_scales;
        probabilities = reshape(prob_im, [r*c, nr_labels]);
    end

    function [probabilities,dictprob] = multiscale_multiply(labelings)
        nr_scales = length(mappings);
        labelings(~any(labelings,2),:) = 1/nr_labels;
        label_image = reshape(labelings, [r,c,nr_labels]);
        prob_im = ones(r,c,nr_labels);
        dictprob = cell(nr_scales,1);
        for i = 1:nr_scales
            label_image_sc = imresize(label_image, mappings(i).scale, 'nearest');
            [r_scale,c_scale,~] = size(label_image_sc);
        ...
        s_pim = sum(prob_im,3);
        s_pim(s_pim == 0) = 1;
        prob_im = prob_im./s_pim;
        probabilities = reshape(prob_im, [r*c, nr_labels]);
    end


    function [probabilities,dictprob] = multiscale_two(labelings)
        nr_scales = length(mappings);
        ...
        max_prob_im = max(prob_im,[],3);
        label_image = double(prob_im == repmat(max_prob_im, [1,1,nr_labels]));
        uncertain = sum(label_image,3) > 1;
        label_image(repmat(uncertain,[1,1,nr_labels])) = 1/nr_labels;
    ...
    function [probabilities,dictprob] = two_weighted(labelings)
        ...
        for i = 1:nr_scales
            ...
            label_vec_sc = reshape(label_image_sc, [r_scale*c_scale, nr_labels]);
            dictprob{i} = mappings(i).T1*label_vec_sc;
            dictprob1 = dictprob{i};
            dictprob1(dictprob{i}==0) = 1;
            w = 1+1/log(nr_labels)*sum(dictprob{i}.*log(dictprob1),2);
            w(w<0) = 0;
            dictprob{i} = dictprob{i}.*(w*ones(1,nr_labels));
            probabilities_sc = mappings(i).T2*dictprob{i}; % computing probabilities
            prob_im_sc = reshape(probabilities_sc, [r_scale, c_scale, nr_labels]);
            prob_im = prob_im + imresize(prob_im_sc, [r,c]);
        end
        ...
        id = find(ps > 0);
        probabilities(id,:) = probabilities(id,:)./ps(id);
        probabilities(probabilities>1) = 1;
        probabilities(probabilities<0) = 0;
    end


    function [probabilities,dictprob] = two_weighted_multiply(labelings)
        nr_scales = length(mappings);
        % DISTRIBUTED tresholded and repeated
        labelings(~any(labelings,2),:) = 1/nr_labels; % distributing
