/*=================================================================
* syntax: Q = probability_search_features_multip(I, T, P, M, b, n); OR Q = probability_search_features(I, T, P, M, b);
*
* serach_km_tree  - build probability image from dictionary and intensity image
* 			
* 			Input: 	- I: X-by-Y-by-l image.
* 					- T: NNl-by-K tree matrix where l is the number of layers 
*                        in the image (1 for grayscale, 3 for RGB, and larger 
*                        for a feature image). N is the patch size of the image.
*                        Typically N = 1 for feature images.
*                   - P: MMc-by-K probability matrix where c is the number of 
*                        classes. M is the patch size of the probability image.
*                   - M: Patch size of probability image.
*                   - b: branching factor.
*                   - n: normalization (true or false), defaults to false.
*
* 			Output: - Q: X-by-Y-by-c probability image
*
* 			Author: Anders Dahl, abda@dtu.dk, November 2017.
*=================================================================*/
...
struct tree_st
{
    ...
    int n_dim, n_dprob, n_nodes, branch_fac, N, M, Mh, n_cls;
    vector<int> idx_disp;
};
...
// vector<double> sample_patch(im_st& im, int& N, int r_im, int c_im, bool& normalize)
vector<double> sample_patch(im_st& im, int& N, int r_im, int c_im, bool& normalize)
{
    ...
    int n_dim = N*N*im.layers; // number of dimensions computed here, becasue tree is not included
    vector<double> patch(n_dim);
    int Nh = (N-1)/2;
    
    for ( int l = 0; l < im.layers; l++ ){ // image is sampled by three nested loops (layers, columns, rows)
        id_l = im.n_pix*l;
        for ( int i = c_im-Nh; i <= c_im+Nh; i++ ){
            id_r = id_l + i*im.rows;
            for ( int j = r_im-Nh; j <= r_im+Nh; j++ ){
                id_i = id_r + j;
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  double *I, *tree, *P, *Q;
  int b, M, N, ndim, ndtree, ndprob;
  //const int *dim, *dtree, *dprob;
  const mwSize *dim, *dtree, *dprob;
  ...
  */
  ...
  dim = mxGetDimensions(prhs[0]); // Image dimensions
  dtree = mxGetDimensions(prhs[1]); // Tree dimensions
  dprob = mxGetDimensions(prhs[2]); // Probability dictionary dimensions
...
//   aout[1] = Im.cols;
  ...
  Q = mxGetPr(plhs[0]);
  for ( int i = 0; i < Im.rows*Im.cols*Tree.n_cls; i++ )
      *(Q+i) = 1;
